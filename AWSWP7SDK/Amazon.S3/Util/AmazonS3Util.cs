...
namespace Amazon.S3.Util
{
    ...
    public static class AmazonS3Util
    {
        ...
        static AmazonS3Util()
        {
            extensionToMime = extensionToMime = new Dictionary<string, string>(150, StringComparer.OrdinalIgnoreCase);
        ...
        public static Dictionary<string, string> CreateHeaderEntry(string key, string value)
        {
        ...
        public static bool ValidateV2Bucket(string bucketName)
        {
            ...
            if (bucketName.StartsWith("s3.amazonaws.com", StringComparison.OrdinalIgnoreCase))
            {
                return false;
            }
            ...
            int idx = bucketName.IndexOf(".s3.amazonaws.com", StringComparison.OrdinalIgnoreCase);
            if (idx > 0)
            {
            ...
            if (bucketName.Length < S3Constants.MinBucketLength ||
                 ...
                 bucketName.StartsWith(".", StringComparison.OrdinalIgnoreCase) ||
                 bucketName.EndsWith(".", StringComparison.OrdinalIgnoreCase))
            {
                return false;
            }
        ...
        /// <param name="client">The Amazon S3 Client to use for S3 specific operations.</param>
        /// <returns></returns>
        public static bool DoesS3BucketExist(string bucketName, AmazonS3 client)
        {
            if (client == null)
            {
                throw new ArgumentNullException("client", "The s3Client cannot be null!");
            }
            ...
            Uri url = new Uri(client.GetPreSignedURL(request));
            ...
            try
            {
                httpRequest.BeginGetResponse(asyncResponse =>
                {
                    HttpWebResponse httpResponse = (HttpWebResponse)httpRequest.EndGetResponse(asyncResponse);
