...
namespace Amazon.SimpleDB.Util
{
    ...
    public static class AmazonSimpleDBUtil
    {
        ...
        /// <param name="maxNumberOfDigits">maximum number of digits in the largest value in the data set</param>
        /// <returns>A string representation of the zero-padded integer</returns>
        public static string EncodeZeroPadding(int number, int maxNumberOfDigits)
        {
            return number.ToString(CultureInfo.InvariantCulture).PadLeft(maxNumberOfDigits, '0');
        }
        ...
        public static string EncodeZeroPadding(float number, int maxNumberOfDigits)
        {
            string fltStr = number.ToString(CultureInfo.InvariantCulture);
            int decPt = fltStr.IndexOf('.');
            if (decPt == -1)
            {
                return fltStr.PadLeft(maxNumberOfDigits, '0');
            }
            else
            {
                return fltStr.PadLeft(maxNumberOfDigits + (fltStr.Length - decPt), '0');
            }
        }
        ...
        /// <param name="maxNumberOfDigits">maximum number of digits in the largest absolute value in the data set</param>
        ...
        public static string EncodeRealNumberRange(int number, int maxNumberOfDigits, int offsetValue)
        {
            return (number + offsetValue).ToString(CultureInfo.InvariantCulture).PadLeft(maxNumberOfDigits, '0');
        }
        ...
        [method: System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Naming", "CA1720:IdentifiersShouldNotContainTypeNames")]
        ...
        /// This method also decodes any encoded properties of the Attribute
